...
alias SecretKeyLength = crypto_secretbox_KEYBYTES;
alias SecretNonceLength = crypto_secretbox_NONCEBYTES;
...
ubyte[] encryptSecretBox(in ubyte[] message,
                         in ubyte[SecretNonceLength] nonce,
                         in ubyte[SecretKeyLength] key) {
...
ubyte[] decryptSecretBox(in ubyte[] cipher,
                         in ubyte[SecretNonceLength] nonce,
                         in ubyte[SecretKeyLength] key) in {
 ...
 */
ubyte[SecretNonceLength] makeNonce() {
    ubyte[SecretNonceLength] nonce = void;
    fillRandom(nonce);
    return nonce;
}
 ...
 */
ubyte[SecretKeyLength] makeSecretKey() {
    ubyte[SecretKeyLength] key = void;
    fillRandom(key);
    return key;
}
